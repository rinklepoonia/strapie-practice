...
export default async function AdmissionDetailPage({
  ...
  // let data: any = null;
  const admissionDetail = await getAdmission(slug);

  console.log(admissionDetail.data, "checkdata");
  const studentDetails = admissionDetail.data
    ...
    return (
      <div className="p-8 text-center mt-12 bg-gray-50 rounded-2xl max-w-2xl mx-auto border border-gray-200">
        ...
        <p className="text-gray-500 mt-2">
          The record you are looking for does not exist or fetch failed.
        </p>
        <Link
          href="/admissions-card"
      ...
      <div className="bg-white border border-gray-300 shadow-xl rounded-2xl overflow-hidden print:shadow-none print:border-none print:my-0">
        ...
        <div className="bg-blue-600 px-8 py-8 flex justify-between items-center text-white print:bg-blue-600 print:-webkit-print-color-adjust-exact">
          <div>
            ...
            <p className="opacity-90 mt-1 text-base">
              Academic Year {new Date().getFullYear()}-
              {new Date().getFullYear() + 1}
            </p>
          </div>
          <button className="hidden sm:block cursor-pointer bg-white text-blue-600 px-5 py-2.5 rounded-lg font-semibold shadow-sm hover:shadow-md transition print:hidden">
        ...
        <div className="px-8 mt-10 space-y-10 text-gray-800 pb-10">
          ...
          <section>
            <h2 className="text-2xl font-bold border-b-2 border-gray-200 pb-3 mb-6 text-gray-900">
              Student Profile
            </h2>
            <div className="flex flex-col md:flex-row gap-8 items-start">
              <div className="w-36 h-36 bg-gray-50 rounded-xl border border-gray-200 shadow-sm flex items-center justify-center overflow-hidden flex-shrink-0">
                {studentDetails.studentImage ? (
                  <img
                    src={studentDetails.studentImage}
                    alt="Student"
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <span className="text-5xl font-bold text-blue-500 opacity-60">
                    {(studentDetails.firstName?.[0] || "S").toUpperCase()}
                  </span>
                )}
              </div>
              ...
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8 w-full mt-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-xl mt-1 text-gray-900 capitalize">
          ...
          <section>
            ...
            {studentDetails?.guardian?.other?.fullName ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8 bg-blue-50/50 p-6 rounded-xl border border-blue-100">
                ...
                <div>
                  ...
                  <p className="font-semibold text-xl mt-1 text-gray-900">
                    {studentDetails.guardian.other.relation}
                  </p>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-8 bg-gray-50 p-6 rounded-xl border border-gray-100">
                <div>
                  ...
                  <p className="font-semibold text-xl mt-1 text-gray-900">
                    {studentDetails?.guardian?.fatherName || "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">
                  ...
                  <p className="font-semibold text-xl mt-1 text-gray-900">
                    {studentDetails?.guardian?.motherName || "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">
                  ...
                  <p className="font-semibold text-lg mt-1 text-gray-800">
                    {studentDetails?.guardian?.fatherQualification || "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">
                  ...
                  <p className="font-semibold text-lg mt-1 text-gray-800">
                    {studentDetails?.guardian?.MotherQualification || "N/A"}
                  </p>
                </div>
              </div>
            )}
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
              <div>
                ...
                <p className="font-semibold text-xl mt-1 text-gray-900">
                  {studentDetails?.address?.village || "N/A"}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">
                ...
                <p className="font-semibold text-xl mt-1 text-gray-900">
                  {studentDetails?.address?.district || "N/A"}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">
                ...
                <p className="font-semibold text-xl mt-1 text-gray-900">
                  {studentDetails?.address?.state || "N/A"}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">
                ...
                <p className="font-semibold text-xl mt-1 text-gray-900">
                  {studentDetails?.address?.PinCode || "N/A"}
                </p>
              </div>
            </div>
          </section>
          ...
          <div className="mt-16 pt-16 border-t-2 border-gray-200 flex flex-col sm:flex-row justify-between items-end text-sm text-gray-500 gap-8">
            <div className="w-full sm:w-auto text-center sm:text-left">
              ...
              <p className="mt-1 text-xs">Document ID: {slug}</p>
            </div>
